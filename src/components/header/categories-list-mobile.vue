...
<template>
  <section class="fixed inset-0 z-10 flex h-screen w-screen bg-700/50 md:hidden">
    <aside
      class="fixed left-0 top-0 h-screen w-80 animate-open overflow-y-auto bg-900 shadow-2xl shadow-700 scrollbar-hide">
      ...
      <nav class="flex w-full items-center justify-center">
        <ul class="flex w-full flex-col items-start justify-between gap-4 p-4">
          <li @click="store.setCategory(category.slug); $emit('close')" v-for="category in TendenciesCategories" :key="category.slug"
